...
;
  ...
  setLoading(true);
  try {
    // Build collected object from all previous user messages
    const collected: Record<string,string> = {};
    messages.forEach(msg => {
      if(msg.role === "user" && msg.ui){
        collected[msg.ui] = msg.content;
    ...
    const result = await axios.post("/api/aimodel", {
      step: "final",
      answer: "",       // not needed, AI uses collected
      isFinal: true,
      collected        // pass the full collected object!
    });
            ...
            value={userInput}
             onKeyDown={(e) => {
              if (e.key === "Enter" && !e.shiftKey) {
                ...
                sendHandle();         // same as send button
              }
  }}
          />
